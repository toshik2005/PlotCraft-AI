...
export function LoginForm({
  ...
  const handleEmailSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${BACKEND_API_URL}/send-otp`, {
        method: "POST",
        headers: {
  ...
  const handleOtpSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.exists) {
        // User exists, create NextAuth session and redirect to dashboard
        toast.success("Successfully logged in!", { description: "Welcome back to RapidRecruit 🚀" })
        
        // Create a NextAuth session by calling the signIn function
        // We'll use a custom provider that handles OTP authentication
        console.log("Attempting to create NextAuth session with:", { email, otp })
    ...
    >
      ...
      <div className="flex justify-end w-full mb-2">
        <button
          ...
          onClick={toggleTheme}
          className="rounded-full p-2 border border-border bg-background hover:bg-muted transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
        >
          {theme === "dark" ? (
            // Sun icon
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="5" fill="currentColor"/><g stroke="currentColor" strokeWidth="2"><path d="M12 1v2"/><path d="M12 21v2"/><path d="M4.22 4.22l1.42 1.42"/><path d="M18.36 18.36l1.42 1.42"/><path d="M1 12h2"/><path d="M21 12h2"/><path d="M4.22 19.78l1.42-1.42"/><path d="M18.36 5.64l1.42-1.42"/></g></svg>
          ) : (
            // Moon icon
            <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M21 12.79A9 9 0 0 1 12.79 3a7 7 0 1 0 8.21 9.79z"/></svg>
          )}
        </button>
      </div>
